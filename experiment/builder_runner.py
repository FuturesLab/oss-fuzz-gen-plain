...
class BuilderRunner:
    ...
    def __init__(
        ...
        containers: Optional[list[ProjectContainerTool]] = None,
    ):
    ...
    def _get_minimum_func_name(self, func_sig: str) -> str:
        ...
        pattern = (
            r"(?:[a-zA-Z_]\w*::)*([a-zA-Z_]\w*|operator[^(\s]*)(?:\s*<.*>)?" r"\s*\("
        )
    ...
    def _contains_target_function(self, target_path: str) -> bool:
        ...
        min_func_name = self._get_minimum_func_name(self.benchmark.function_signature)
    ...
    def _pre_build_check(self, target_path: str, build_result: BuildResult) -> bool:
        ...
        if not result:
            ...
            logger.warning(
                "Missing target function: %s does not contain %s",
                target_path,
    ...
    def _parse_stacks_from_libfuzzer_logs(self, lines: list[str]) -> list[list[str]]:
        ...
        for line in lines:
            is_stack_frame_line = (
                LIBFUZZER_STACK_FRAME_LINE_PREFIX.match(line) is not None
            )
    ...
    def _parse_func_from_stacks(
        self, project_name: str, stacks: list[list[str]]
    ) -> dict:
            ...
            for line in stack:
                ...
                if func_name == "LLVMFuzzerTestOneInput":
                    ...
                    if line_match:
                        ...
                        logger.warning(
                            "Failed to parse line number from %s in project %s",
                            func_name,
                            project_name,
                        )
                    break
    ...
    def _stack_func_is_of_testing_project(self, stack_frame: str) -> bool:
        return (
            bool(CRASH_STACK_WITH_SOURCE_INFO.match(stack_frame))
            and LIBFUZZER_LOG_STACK_FRAME_LLVM not in stack_frame
            and LIBFUZZER_LOG_STACK_FRAME_LLVM2 not in stack_frame
            and LIBFUZZER_LOG_STACK_FRAME_CPP not in stack_frame
        )

    def _parse_libfuzzer_logs(
        self, log_handle, project_name: str, check_cov_increase: bool = True
    ) -> ParseResult:
        ...
        if crashes:
            ...
            if symptom == "signal":
                return ParseResult(
                    ...
                    artifact_name,
                    SemanticCheckResult(
                        SemanticCheckResult.SIGNAL, symptom, crash_stacks, crash_func
                    ),
                )
            ...
            if symptom.endswith("fuzz target exited"):
                return ParseResult(
                    ...
                    artifact_name,
                    SemanticCheckResult(
                        SemanticCheckResult.EXIT, symptom, crash_stacks, crash_func
                    ),
                )
            ...
            if "out-of-memory" in symptom or "out of memory" in symptom:
                return ParseResult(
                    ...
                    artifact_name,
                    SemanticCheckResult(
                        SemanticCheckResult.FP_OOM, symptom, crash_stacks, crash_func
                    ),
                )
    ...
    def _copy_crash_file(
        self, outdir: str, artifact_dir: str, run_result: RunResult
    ) -> None:
    ...
    def build_and_run(
        ...
        try:
            return self.build_and_run_local(
                generated_project, target_path, iteration, build_result, language, trial
            )
    ...
    def build_and_run_local(
        ...
        build_result.succeeded = self.build_target_local(
            generated_project, benchmark_log_path
        )
        if not build_result.succeeded:
            errors = code_fixer.extract_error_message(
                benchmark_log_path, project_target_name, language
            )
        ...
        with open(run_log_path, "rb") as f:
            ...
            (
                run_result.cov_pcs,
                run_result.total_pcs,
                run_result.crashes,
                run_result.crash_info,
                run_result.artifact_name,
    ...
    def run_target_local(
        ...
        if proc.returncode != 0:
            ...
            logger.info("Failure logs stored at: %s", log_path)
        else:
            logger.info("Successfully run %s.", generated_project)

    def _build_target_with_reuse(self, sanitizer: str, log_path: str) -> bool:
        proc = None
        if sanitizer == "address":
            container = self.containers[0]
            logger.info(
                "**** Building address container for reuse **** %s",
                container.container_id,
            )
        elif sanitizer == "coverage":
            container = self.containers[1]
            logger.info(
                "**** Building coverage container for reuse **** %s",
                container.container_id,
            )
        else:
        ...
        logger.info(
            "Successfully build fuzzer for %s with %s",
            container.generated_oss_fuzz_name,
            sanitizer,
        )
        return True

    def build_target_local(
        self, generated_project: str, log_path: str, sanitizer: str = "address"
    ) -> bool:
        """Builds a target with OSS-Fuzz."""

        logger.info("Building %s with %s", generated_project, sanitizer)

        # Reuse containers if we have them
        if self.containers is not None:
            return self._build_target_with_reuse(sanitizer, log_path)
        ...
        if oss_fuzz_checkout.ENABLE_CACHING and oss_fuzz_checkout.is_image_cached(
            ...
            # Prepare build
            oss_fuzz_checkout.prepare_build(
                self.benchmark.project, sanitizer, generated_project
            )
        ...
        command = [
            ...
            os.path.join(oss_fuzz_checkout.OSS_FUZZ_DIR, "projects", generated_project),
        ]
        ...
        with open(log_path, "w+") as log_file:
            try:
                ...
                logger.info(
                    "Failed to build fuzzer for %s with %s",
                    generated_project,
                    sanitizer,
                )
                return False
        ...
        logger.info(
            "Successfully build fuzzer for %s with %s", generated_project, sanitizer
        )
        return True
    ...
    def _get_coverage_text_filename(self, project_name: str) -> str:
        ...
        return os.path.join(
            get_build_artifact_dir(project_name, "out"),
            "textcov_reports",
    ...
    def _extract_local_textcoverage_data(self, project_name: str) -> textcov.Textcov:
        ...
        with open(
            local_textcov_location, language_modes.get(self.benchmark.language, "rb")
        ) as f:
            if self.benchmark.language == "jvm":
    ...
    def get_coverage_local(
        self, generated_project: str, benchmark_target_name: str
        ...
        log_path = os.path.join(self.work_dirs.build_logs, f"{sample_id}-coverage.log")
        logger.info("Building project for coverage")
        built_coverage = self.build_target_local(
            generated_project, log_path, sanitizer="coverage"
        )
...
class CloudBuilderRunner(BuilderRunner):
    ...
    def build_and_run(
        ...
        try:
            return self.build_and_run_cloud(
                generated_project,
                target_path,
                iteration,
                build_result,
    ...
    def build_and_run_cloud(
        ...
        logger.info(
            "Servie account key: %s", os.environ.get("GOOGLE_APPLICATION_CREDENTIALS")
        )
        command = [
        ...
        if not self._run_with_retry_control(
            os.path.realpath(target_path), command, cwd=oss_fuzz_checkout.OSS_FUZZ_DIR
        ):
            return build_result, None
        ...
        with open(
            self.work_dirs.build_logs_target(generated_target_name, iteration, trial),
            "wb",
        ) as f:
            blob = bucket.blob(build_log_name)
            if blob.exists():
                logger.info(
                    "Downloading cloud build log of %s: %s to %s",
                    os.path.realpath(target_path),
                    build_log_name,
                    f,
                )
                ...
                logger.warning(
                    "Cannot find cloud build log of %s: %s",
                    os.path.realpath(target_path),
                    build_log_name,
                )
        ...
        with open(run_log_path, "wb") as f:
            ...
            if blob.exists():
                ...
                logger.info(
                    "Downloading cloud run log of %s: %s to %s",
                    os.path.realpath(target_path),
                    run_log_name,
                    f,
                )
                ...
                logger.warning(
                    "Cannot find cloud run log of %s: %s",
                    os.path.realpath(target_path),
                    run_log_name,
                )
        ...
        if not build_result.succeeded:
            ...
            build_result.errors = errors
            logger.info(
                "Cloud evaluation of %s indicates a failure: %s",
                os.path.realpath(target_path),
                errors,
            )
            return build_result, None
        logger.info(
            "Cloud evaluation of %s indicates a success.", os.path.realpath(target_path)
        )
        ...
        for blob in blobs:
            ...
            local_path = os.path.join(
                code_coverage_report_dir, "report", "linux", relative_path
            )
        ...
        if self.benchmark.language == "jvm":
            ...
            if blob.exists():
                ...
                self._copy_textcov_to_workdir(
                    bucket, textcov_blob_path, generated_target_name
                )
        elif self.benchmark.language == "python":
            blob = bucket.blob(textcov_blob_path)
            if blob.exists():
        ...
        if blobs:
            ...
            logger.warning(
                "Cloud evaluation of %s failed to downlod artifact:%s",
                os.path.realpath(target_path),
    ...
    def _copy_textcov_to_workdir(
        self, bucket, textcov_blob_path: str, generated_target_name: str
    ) -> None:
    ...
    def _get_cloud_textcov_path(self, coverage_name: str) -> str:
        ...
        # For C/C++/Rust
        return (
            f"{coverage_name}/textcov_reports/{self.benchmark.target_name}" ".covreport"
        )
