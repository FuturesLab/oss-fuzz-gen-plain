...
JVM_SKIPPED_METHOD = ["fuzzerTestOneInput", "fuzzerInitialize", "fuzzerTearDown"]
...
class Textcov:
    ...
    def _read_file_with_fallback(
        cls, file_handle: BinaryIO, sample_size: int = 1000
    ) -> str:
        ...
        try:
            ...
            if encoding is None:
                ...
                raise UnicodeDecodeError(
                    "chardet", raw_data, 0, len(raw_data), "Cannot detect encoding"
                )
    ...
    @classmethod
    def from_file(
        cls, file_handle, ignore_function_patterns: Optional[List[re.Pattern]] = None
    ) -> Textcov:
        ...
        for line in demangled.split("\n"):
            ...
            if match:
                ...
                if line_contents in current_function.lines:
                    ...
                    current_function.lines[line_contents] = Line(
                        contents=line_contents, hit_count=hit_count
                    )
                ...
                for method_item in item:
                    if method_item.tag == "method":
                        if method_item.attrib["name"] not in JVM_SKIPPED_METHOD:
                            class_method_items.append(
                                (class_name, method_item, coverage)
                            )
