...
from experiment.container_pool import ContainerPool
from experiment.workdir import WorkDirs
from llm_toolkit import models, output_parser, prompt_builder, prompts
from results import BenchmarkResult, Result, TrialResult, AnalysisResult
...
class AggregatedResult:
    ...
    def from_benchmark_result(
        cls, benchmark_result: BenchmarkResult
    ) -> "AggregatedResult":
...
def generate_targets(
    ...
    logging.info(
        "Generating targets for %s %s using %s..",
        benchmark.project,
...
def aggregate_results(
    target_stats: list[tuple[int, exp_evaluator.Result]], generated_targets: list[str]
) -> AggregatedResult:
    ...
    crash_rate = sum([int(stat.crashes) for _, stat in target_stats]) / len(
        target_stats
    )
    found_bug = sum(
        [int(stat.crashes and not stat.is_semantic_error) for _, stat in target_stats]
    )
    max_coverage = max([stat.coverage for _, stat in target_stats])
    max_line_coverage_diff = max([stat.line_coverage_diff for _, stat in target_stats])
...
def check_targets(
    ...
    if cloud_experiment_name:
        ...
        builder_runner = builder_runner_lib.BuilderRunner(
            benchmark, work_dirs, run_timeout, fixer_model_name
        )
    ...
    with pool.ThreadPool(NUM_EVA) as p:
        for i, target_stat in enumerate(
            ...
            if target_stat is None:
                logging.error(
                    "This should never happen: Error evaluating target: %s",
                    generated_targets[i],
                )
...
def store_successful_harness(
    ...
    artifact_path = os.path.join(
        shared_corpus_path, str("".join(random.choices(string.ascii_lowercase, k=10)))
    )
...
def _fuzzing_pipeline(
    ...
    container_pair = CONTAINER_POOL.get_container_pair()
    containers = [container_pair.address_container, container_pair.coverage_container]
    ...
    if args.custom_pipeline == "function_based_prototyper":
        p = pipeline.Pipeline(
            ...
            writing_stage_agents=[
                FunctionToolPrototyper(trial=trial, llm=model, args=args),
                FunctionToolPrototyper(trial=trial, llm=model, args=args),
            ],
        ...
        if "gemini" in args.model or "vertex" in args.model:
            writer_agents.append(
                FunctionAnalyzer(trial=trial, llm=model, args=args, benchmark=benchmark)
            )
        writer_agents += [
        ...
        p = pipeline.Pipeline(
            ...
            analysis_stage_agents=[
                ...
                ContextAnalyzer(trial=trial, llm=model, args=args, benchmark=benchmark),
            ],
            containers=containers,
        )
    else:
    ...
    results = p.execute(
        result_history=[Result(benchmark=benchmark, trial=trial, work_dirs=work_dirs)]
    )
    ...
    CONTAINER_POOL.release_container_pair(container_pair)
    return trial_result


def _fuzzing_pipelines(
    benchmark: Benchmark,
    model: models.LLM,
    args: argparse.Namespace,
    work_dirs: WorkDirs,
    ...
    with pool.ThreadPool(processes=NUM_EVA) as p:
        ...
        task_args = [
            (benchmark, model, args, work_dirs, trial)
            for trial in range(1, args.num_samples + 1)
        ]
        trial_results = p.starmap(_fuzzing_pipeline, task_args)
    return BenchmarkResult(
        benchmark=benchmark, work_dirs=work_dirs, trial_results=trial_results
    )
