...
def generate_benchmarks(args: argparse.Namespace) -> None:
    ...
    projects_to_target = [
        project.strip() for project in args.generate_benchmarks_projects.split(",")
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=TEMPERATURE,
        help=(
            "A value between 0 and 1 representing the variety of the targets "
            "generated by LLM."
        ),
    )
    parser.add_argument(
        ...
        default=[],
        help=(
            "A list of values representing the temperatures will be used by "
    ...
    parser.add_argument(
        "-y", "--benchmark-yaml", type=str, help="A benchmark YAML file."
    )
    ...
    parser.add_argument(
        ...
        "--generate-benchmarks",
        help=(
            "Generate benchmarks and use those for analysis. This is a string "
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Delay each experiment by certain seconds (e.g., 10s) to avoid "
    ...
    if benchmark_yaml:
        assert benchmark_yaml.endswith(".yaml") or benchmark_yaml.endswith(
            "yml"
    ...
    assert bool(args.cloud_experiment_name) == bool(args.cloud_experiment_bucket), (
...
def extend_report_with_coverage_gains() -> None:
    ...
    for language, lang_cov_gain in total_new_covgains.items():
        try:
            total_coverage_increase = round(
                (lang_cov_gain / existing_oss_fuzz_cov[language]["total"]) * 100.0, 10
            )
        ...
        try:
            relative_coverage_increase = round(
                (lang_cov_gain / existing_oss_fuzz_cov[language]["covered"]) * 100.0, 10
            )
        except (KeyError, ZeroDivisionError):
    ...
    add_to_json_report(WORK_DIR, "coverage_gains_per_language", total_new_covgains)
    add_to_json_report(WORK_DIR, "project_summary", coverage_gain_dict)
    add_to_json_report(WORK_DIR, "oss_fuzz_language_status", existing_oss_fuzz_cov)
    add_to_json_report(WORK_DIR, "comperative_coverage_gains", comparative_cov_gains)
...
def _print_experiment_result(result: Result):
    ...
    logger.info(
        ...
        result.benchmark.function_signature,
        result.result,
    )


def _print_experiment_results(
    results: list[Result], cov_gain: dict[str, dict[str, Any]]
):
...
def _process_total_coverage_gain() -> dict[str, dict[str, Any]]:
    ...
    for benchmark_dir in os.listdir(WORK_DIR):
        ...
        coverage_reports = os.path.join(
            WORK_DIR, benchmark_dir, "code-coverage-reports"
        )
        ...
        for sample in os.listdir(coverage_reports):
            ...
            for textcov_file in os.listdir(summary):
                if textcov_file.endswith(".covreport"):
                    ...
                    with open(os.path.join(summary, textcov_file)) as f:
                        textcov_dict[project_name].append(
                            textcov.Textcov.from_jvm_file(f)
                        )
...
def main():
    ...
    start = time.time()
    add_to_json_report(
        args.work_dir, "start_time", time.strftime(TIME_STAMP_FMT, time.gmtime(start))
    )
    ...
    # This line is the only logic that differs from original Oss-Fuzz-Gen
    run_one_experiment.setup_container_pool(experiment_targets[-1], NUM_EXP)

    logger.info(
        "Running %s experiment(s) in parallels of %s.",
        len(experiment_targets),
        str(NUM_EXP),
    )
    ...
    add_to_json_report(
        args.work_dir, "total_run_time", str(timedelta(seconds=end - start))
    )
